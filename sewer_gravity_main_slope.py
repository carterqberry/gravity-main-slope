"""
Updates the upstream (UPELEV) and Downstream(ELEVDOWN) elevations to the SsGravityMain
based on the nearest manhole invert elevations so the slope of the pipe can be calculated
Process:    1. Create start and end points for each gravity main pipe
            2. Join those points with manholes to grab invert elevations using NEAR tool
            3. Write those elevations back to the SsGravityMain 'ELEVUP' & 'ELEVDOWN' fields
...
# SDE Path:
sde_path = r"....\Sewer.sde" #Full path removed for github repository.

# Set environment variables:
arcpy.env.workspace = sde_path # Set the workspace to the geodatabase connection
arcpy.env.qualifiedFieldNames = False # Use simple field names without table prefixes (e.g., "INV" instead of "ssManhole.INV")
...
# Log Path:
log_path = r"....\gravity_main_slope\sewer_gravity_main_slope_log.txt" # Full path removed for github repository.
...
# Input layers:
ssgravity_main = "DBO.ssGravityMain" # Gravity pipe lines
ssmanhole = "DBO.ssManhole" # Manhole point features
# Do we care about any other point features when it comes to elevations for sewer? Grease Traps? Clean Outs? Underground Enclosure?

# ----------------------------------------------------------
# Step 1: Create temporary start and end points in memory:
# ----------------------------------------------------------

print("\nStarting slope update process...")
...
print("Creating start and end points on ssGravityMain segments...")

# Extract the START vertex of each gravity main line and create a point feature:
arcpy.management.FeatureVerticesToPoints(ssgravity_main, start_points, "START") # The start point is the first vertex in the line geometry.
# Extract the END vertex of each gravity main line and create a point feature:
arcpy.management.FeatureVerticesToPoints(ssgravity_main, end_points, "END") # The end point is the first vertex in the line geometry. Reversing line direction will swap these (which is what we want).
...
# --------------------------------------------------------------------------------------------
# Step 2: Create a filtered ssManhole feature layer:
# --------------------------------------------------------------------------------------------

print("\nCreating a filtered feature layer from ssManhole...")

# Filter manholes to only those with valid elevations:
arcpy.MakeFeatureLayer_management(
    ssmanhole,                  # Input: ssManhole feature class
...
# Find the nearest sewer node (manhole) to each start point within 3 feet:
arcpy.Near_analysis(start_points, "filtered_nodes", search_radius = "3 FEET")
# Find the nearest sewer node (manhole) to each end point within 3 feet:
...
# ---------------------------------------------------------------
# Step 5: Update ssGravityMain layer with new elevation values
# ---------------------------------------------------------------

print("\nUpdating ssGravityMain with new elevation values...")
...
with arcpy.da.UpdateCursor(ssgravity_main, ["OBJECTID", "ELEVUP", "ELEVDOWN"]) as cur:
...
with arcpy.da.UpdateCursor(ssgravity_main, ["OBJECTID", "ELEVUP", "ELEVDOWN", "SHAPE@LENGTH", "Slope"]) as cur:
...
with arcpy.da.SearchCursor(ssgravity_main, ["ELEVUP", "ELEVDOWN"]) as cur:
    for up, down in cur:
...
if updated_count > 0:
    writeToLog(f"Sewer gravity main elevations updated successfully.\n")
    writeToLog(f"Records updated: {updated_count}\n")
    writeToLog(f"Negative slopes: {negative_slope_count} ({negative_slope_percent:.2f}%)\n\n")
else:
    writeToLog("No sewer gravity main elevation updates needed.\n\n")

# Print summary:
print(f"\n✅ Sewer gravity main elevations updated successfully!")
